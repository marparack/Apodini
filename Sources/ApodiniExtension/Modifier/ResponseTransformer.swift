...
public struct ResponseTransformingHandlerInitializer<T: ResponseTransformer>: DelegatingHandlerInitializer {
    public typealias Response = ApodiniExtension.Response<T.Content>
...
extension TransformerCandidate: Transformable where Transformer.InputContent == Delegate.Response.Content {
    func callAsFunction() -> Any {
        SomeHandler<Response<Transformer.Content>>(ResponseTransformingHandler<Delegate, Transformer>(
                                                    transformed: ApodiniExtension.Delegate(delegate),
                                                    transformer: ApodiniExtension.Delegate(transformer)))
    }
}
