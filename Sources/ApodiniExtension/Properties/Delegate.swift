...
public struct Delegate<D> {
    ...
    public func callAsFunction() throws -> D {
        ...
        // if not done yet we _activate the delegate before injection
        if !store.value.didActivate {
            store.value.didActivate = true
            ApodiniExtension.activate(&store.value.delegate)
        }
...
extension Delegate: _Activatable {
    mutating public func _activate() {
