...
struct LIFXDeviceDiscoveryAction: PostDiscoveryAction {
    ...
    @Configuration(IoTUtils.resourceDirectory)
    var resourceURL: URL
    
    @Configuration(.username)
    var username: String
    
    @Configuration(IoTUtils.deploymentDirectory)
    var deploymentDir: URL
    
    @Configuration(IoTUtils.logger)
    var logger: Logger
    ...
    func run(_ device: Device, on eventLoopGroup: EventLoopGroup, client: SSHClient?) throws -> EventLoopFuture<Int> {
        ...
        guard let sshClient = client else {
            return eventLoop.makeFailedFuture(IoTDeploymentError.generic("Could not find ssh client. Check if you provided the necessary credentials in the config."))
        }
        ...
        guard FileManager.default.fileExists(atPath: scriptFileUrl.path) else {
            throw IoTDeploymentError.generic("Unable to find '\(setupScriptFilename)' resource in bundle")
        }

        // Create tmp sub dir in deployment dir for results
        try sshClient.fileManager.createDir(on: tmpLifxDir, permissions: 777)
        
        try IoTUtils.copyResourcesToRemote(device, origin: scriptFileUrl.path, destination: IoTUtils.rsyncHostname(device, username: username, path: tmpLifxDir.path))
        
        logger.info("executing script")
        sshClient.assertSuccessfulExecution(cmd: "bash \(tmpLifxDir.appendingPathComponent(setupScriptFilename)) \(tmpLifxDir.path)")
        ...
        try IoTUtils.copyResourcesToRemote(
            device, origin: IoTUtils.rsyncHostname(device, username: username, path: remoteResultsPath.path), destination: resourceURL.path)
        ...
        guard let data = FileManager.default.contents(atPath: resultsPath) else {
            throw IoTDeploymentError.generic("Could not find results file at \(scriptFileUrl)")
        }
